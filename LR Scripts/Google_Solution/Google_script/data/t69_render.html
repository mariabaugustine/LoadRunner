...
var ComponentViewModel = function (args) {
			...
			self.langs = ko.observable('');
			self.langsFilter = ko.observable({
				"de":"german",
				"en":"english",
				"es":"spanish",
				"fr":"french",
			...
			self.getSearchURL = function(){
				...
				if(envURL.indexOf('orasitestest') >= 0 || envURL.indexOf('www-stage') >= 0){
					url += 'https://search-stage.oracle.com';
				}else{
					url += 'https://search.oracle.com';
				}
				if(SCS.pageLanguageCode){

					self.filter(self.langsFilter()[SCS.pageLanguageCode.toLowerCase()]);

					if(SCS.pageLanguageCode.toLowerCase() == 'en'){
						self.langs('en');
						url += `/java`
					}else if(SCS.pageLanguageCode.toLowerCase() == 'pt-br'){
						self.langs('br');
						url += `/br/java`
					}else if(SCS.pageLanguageCode.toLowerCase() == 'zh-cn'){
						self.langs('cn');
						url += `/cn/java`
					}else if(SCS.pageLanguageCode.toLowerCase() == 'zh-tw'){
						self.langs('tw');
						url += `/tw/java`
					}else{
						self.langs(SCS.pageLanguageCode);
						url += `/${SCS.pageLanguageCode}/java`;
					}
				}else{
					url += '/java';
				}
			...
			self.searchURL(self.getSearchURL());
